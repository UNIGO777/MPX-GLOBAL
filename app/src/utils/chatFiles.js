/**
 * Chat attachments on the app (D9 images · D10 documents) — mirrors
 * `web/src/lib/chatFiles.js`. These checks are for the SENDER'S benefit only (a
 * clear message before an 8 MB upload); the server re-checks every file by its
 * real bytes and is the only thing that decides.
 */
...
export function chatFileProblem(file, kind) {
  if (file.size != null && file.size > CHAT_FILE_MAX_MB * 1024 * 1024) {
