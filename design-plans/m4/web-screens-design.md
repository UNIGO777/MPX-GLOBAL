...
| 7 | **Supplier-page "Start Conversation" placeholder vs M4-4** *(added 2026-08-14; owner ruled same day: DEFERRED, KEEP DISABLED)* | `/supplier/:slug` ships a disabled owner-requested "Start Conversation" button (2026-08-13, `docs/UiWebNotes.md`), but every M4 thread is **product-scoped** — there is no supplier-level conversation to wire it to. **Owner decision 2026-08-14: keep the company-level button on the page, disabled, through M4 wiring; how it reconciles with M4-4 will be decided later.** The options when that decision comes: (a) route it to the supplier's catalogue so the buyer picks a product; (b) drop the button; (c) a backend change allowing product-less threads, which contradicts M4-4. Meanwhile the owner also ruled: product-page **"Send Enquiry" is the one door that wires to chat**, and the category-card "Inquiry" button is **deactivated** (removed or kept inert at wiring; see §4 screen 1). Red-flag before designing anything supplier-level. |
