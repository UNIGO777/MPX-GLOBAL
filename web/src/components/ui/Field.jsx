...
/**
 * Shared input classes so every control matches the DESIGN.md spec: 8px
 * radius, 1px #C5C6CF border; focus = accent border + soft accent glow (the
 * buyer-registration mockup's `ring-mpx-glow/20`), NEVER an offset halo;
 * disabled = the mockups' grey fill; error = red border + pale red tint.
 */
export function inputClasses(hasError, extra = '') {
  return [
    'h-11 w-full rounded-lg border px-3.5 text-sm text-ink-900 placeholder:text-ink-500 transition-all',
