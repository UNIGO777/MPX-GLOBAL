...
| **Hard-delete** (🔴 §A5: only the admin §A8 180-day blocked purge — never a seller/draft delete) | **hata do (cleanup)** | list se gaayab |
...
- **Only exception:** admin **§A8** 180-day blocked-product purge (cleanup job, not a seller action).
...
- Admin = **takedown, not hard delete** (M2 locked) — is doc me respect kiya; "delete" ko soft-delete treat kiya. 🔴 **Except Part A §A8**: blocked > 180 days → purge (with audit snapshot).
...
- Seller delete: **always soft** (`status: archived`) — no draft=hard branching (Part A §A5). Only hard delete = admin §A8 180-day blocked-product purge.