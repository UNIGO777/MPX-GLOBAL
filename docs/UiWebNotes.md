...
| 2026-09-25 | `web/src/pages/exporter/QuotationBuilder.jsx` (send modal) + `web/src/pages/account/CompanyProfile.jsx` | Bank details for quotations — **no UI exists at all** | The backend is complete (`ExporterBankAccount`, `GET/POST/PATCH/DELETE /me/bank-accounts`, `/confirm`) and the quotation snapshots whatever is chosen, but **nothing in the web app can add or edit a bank account**, so the picker in the send modal is always empty and every quotation prints without payment details. Needs a section on the exporter's Company profile: list (masked, `••••4444`), add, edit, delete, set default. 🔴 Never render the full account number back — the API does not return it. | Not deferred, just not built — surfaced 2026-09-25 while fixing the builder UI. The modal's copy used to send the exporter to "your company settings" to find a section that does not exist; it now says plainly that it is not built yet. | Pending |
