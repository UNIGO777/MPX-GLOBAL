...
| 2026-08-03 | `web/src/pages/auth/SignupCompany.jsx` + `app/src/screens/auth/SignupCompanyScreen.jsx` | Organisation **claim** path (A21 "we found a company registered with this email") | Offer claim-vs-create against an existing Organisation; a claimed org carries its verification over (one KYC, one tick, one public profile) | No backend endpoint — `/auth/signup/complete` always CREATES. Claim is the remaining half of A21 and was out of scope for the verification fix | **Done (WEB) 2026-09-23**, revised the same day to the seven-rule model (build-prompt §A21 "Organisation claim"): `ClaimOffer.jsx` renders a list with a picker, the `claim_org_email` code step, and swaps to create on `CLAIM_SEAT_TAKEN`; `complete` takes an opaque `claimChoice` — never an org id. The APP half is row 57 (also Done, 2026-09-24). Tests: `d7-organisation-claim.test.js`. |
...
| 2026-08-03 | `web/src/pages/auth/BuyerSignup.jsx` | "Terms of Service" / "Privacy Policy" in the closing fine print | Link to the real legal pages | ✅ **2026-08-23 — DONE.** `/terms` + `/privacy` shipped (`pages/public/Legal.jsx`); the fine print is real links now. The old "enterprise-grade security" sentence was dropped with them — unevidenced marketing on a consent line | Done |
| 2026-08-01 | `web/src/components/public/PublicFooter.jsx` | Company / Resources columns (About, Careers, Contact, Blog, Help Center, Trade Guides) | Real pages | ✅ **2026-08-23 — REMOVED, not fixed.** Six greyed non-links are furniture pretending to be navigation; replaced with a "Discover" column pointing at surfaces that exist. They return in the change that ships the pages. **Legal is now real** (`/privacy`, `/terms`) | Done (columns removed) |
| 2026-08-01 | `web/src/pages/public/Landing.jsx` | Testimonials section (mockup had 6 cards) | Real customer quotes | **DELIBERATELY NOT BUILT** — fabricated people praising unbuilt features (escrow, shipments, analytics). ✅ **2026-08-23 — row CLOSED:** the landing rebuild has no testimonials section at all, so there is nothing pending. A future real-quotes section is new work, not an outstanding gap | Closed (nothing pending) |
...
| 2026-08-01 | `web/src/pages/exporter/exporterNav.js` → `layouts/PortalLayout.jsx` sidebar | "Dashboard" item | Exporter dashboard (later milestone) | Outside the M1 web set; disabled "Soon" chip | **Done 2026-09-22** — `web/src/pages/exporter/Dashboard.jsx` shipped at `/exporter/dashboard`; the row is a real route and `/exporter` redirects to it. ⚠️ Worth knowing how this row ended: it sat dimmed for 7 weeks and, because it *looked* scheduled, it got copied into `docs/Pending-Work.md` as "scheduled month 2" — a feature expectation manufactured by a placeholder, not by the quote (no seller dashboard appears in `docs/scope-of-work.md`). It was red-alerted before building and the owner confirmed on the app-parity argument. **The lesson this ledger exists for: a dimmed row is a promise, and an unowned promise grows a due date.** |
...
| 2026-08-18 | `web/src/pages/admin/Organisations.jsx` (`/admin/organisations`) | Row action **"Open detail"** — NOT rendered | Open the Organisation detail screen (M5 #15): header + block/unblock, company, verification incl. which side was actually reviewed, sides + claim history, users with last login, audit trail, per-side chat counts, exporter products / buyer enquiries | M5 screen 15 is not built and `/admin/organisations/:id` does not exist. Rather than render a link into a missing route, each row offers only destinations that DO exist today — products (`?seller=`), conversations (`?orgId=`) and KYC documents. **Nothing on this screen is dead**; this row records the ABSENT action so the gap is visible | **Done 2026-08-18** — M5 screen 15 shipped the same day; `/admin/organisations/:id` exists and both the row menu and the company name open it |
| 2026-08-01 | `web/src/layouts/AdminLayout.jsx` sidebar → `pages/admin/ComingSoon.jsx` | "Dashboard" item (sidebar) | Real dashboard screen (backend GET /admin/dashboard exists — M5) | Outside the M1 web set. Per the design file this row is non-interactive with a SOON badge — it no longer links to `/admin/dashboard` (that route still renders ComingSoon if reached directly) | **Done 2026-08-18** — M5 screen 12 built; the sidebar item is a real link to `/admin/dashboard` and the SOON chip is gone |
...
| 2026-08-01 | `web/src/layouts/AdminLayout.jsx` sidebar → `pages/admin/ComingSoon.jsx` | "Settings" item + `/admin/settings` route | Platform settings screen (no backend yet) | Design shows Settings as a real link with no badge → it routes to the designed ComingSoon page | **Done 2026-09-22** — `web/src/pages/admin/Settings.jsx` shipped at `/admin/settings` (D8): AI guest daily ceiling + support contact, superadmin-only, every save audited. **This was the LAST `ComingSoon` route in the console** — every admin route now renders a real screen, and the `ComingSoon` lazy binding was removed from `App.jsx` (the component file stays; it also exports `NoAccess`). |
...
| 2026-08-23 | `web/src/pages/public/Legal.jsx` | Terms of Service + Privacy Policy — the "contact address published by MPX Global" both documents point at | A real support / privacy contact address (email or page) | 🔴 **The owner must supply this.** Both documents tell people to contact us to ask about the policy, request their data, or ask for deletion — and there is no address anywhere on the site to do it. A privacy policy whose contact route is circular is the one defect in it that matters. Not invented here on purpose | Pending (owner content) |
| 2026-08-23 | `web/src/pages/public/Legal.jsx` | Both documents + their "interim document" notice | Replace wholesale with the CLIENT's own Terms of Service and Privacy Policy, then remove the notice | 🔴 **Owner decision 2026-08-23: the client supplies the final documents** — ours is a placeholder so nobody is asked to agree to a document that does not exist. ⚠️ **Check the client's text against the software before publishing**: a template will typically promise self-service account deletion, a fixed retention period, and GDPR/DPDP compliance — none of which is true here. Either the document matches the behaviour or the software changes to match it | Pending (client content) |
...
| 2026-08-02 | `app/src/screens/auth/SignupAccountScreen.jsx` | "I agree to the Terms of Service and Privacy Policy" checkbox (in mockup) | Record consent at signup | ✅ **2026-08-23 — partially done.** The screen now STATES the consent and links to the real `/terms` + `/privacy` on the website. Still **no checkbox**, deliberately: the server has no field to record a tick against, so one would imply we store an agreement we do not. 🔴 **Owner decision still open**: whether consent must be captured and stored per account before launch | Pending (owner decision — capture only) |
| 2026-08-02 | `app/src/screens/auth/SignupCompanyScreen.jsx` | Screen 8 **Path A · Claim an existing company** | "We found a company registered with this email" card → claim vs create new | ⏸ **DEFERRED by the owner (2026-08-18) — see `docs/Note.md` D7.** There is no organisation lookup or claim endpoint (`auth.routes.js` has neither); that is the whole remaining blocker. ~~Account-enumeration surface~~ — **withdrawn 2026-08-18**: §A21 line 248 puts step 2 behind BOTH OTPs, so the caller has proved they own the address being matched and learns nothing about a stranger. The one rule to keep: match only on the verified identity, never on a typed company name. Only Path B (create new) ships | ✅ **Done (APP) 2026-09-24 — commit `a6b4116` "Organisation claim in the app"** (`ClaimOffer.jsx` + `SignupCompanyScreen.jsx`: list + picker, `claim_org_email` code, opaque `claimChoice`, `CLAIM_SEAT_TAKEN` → create; rule 7 read-only in `CompanyProfileScreen`/`VerificationHubScreen`). This row said "still a stub" until 2026-09-25 — stale, corrected after the owner asked. Not device-tested. Old note: Backend is ready. When built it must implement **every** rule in build-prompt §A21 "Organisation claim" — list + picker, `claim_org_email` code, name withheld until then, opaque `claimChoice`, `CLAIM_SEAT_TAKEN` → create, rule 7 read-only profile/KYC. `.claude/rules/mobile-app.md` points there. Until then every app signup creates a duplicate organisation. |
...
| 2026-08-17 | `app/src/screens/BuyerHomeScreen.jsx` | Search bar + filter button (header row) | Open M3 app search | ✅ Done 2026-08-19 — M3 shipped; both navigate to the live Search tab (`SearchHomeScreen`) | Done |
...
| 2026-09-25 | `web/src/pages/exporter/QuotationBuilder.jsx` (send modal) + `web/src/pages/account/CompanyProfile.jsx` | Bank details for quotations — no UI existed | ✅ **Done 2026-09-25** — `components/account/BankAccounts.jsx` on the exporter's Company profile: list (masked), add, edit, remove, set default, all against the existing `/me/bank-accounts`. The full number is never shown back (the API does not return it), so an edit re-types it or leaves it blank to keep the stored one. The send modal now links to the section instead of naming one that did not exist. | — | Done |
| 2026-09-26 | `web/src/pages/public/Landing.jsx` | The HERO — rebuilt around AI match-making | The blank band is GONE: the hero now carries the real `<h1>`, an AI search field that submits to `/ai-search?q=…`, three example prompts that run the same search, and the `CircuitHero` backdrop. Every control in it is wired — nothing here is a placeholder. 🔴 **Still missing from the pre-2026-09-26 hero:** the guest signup cards ("Start sourcing" / "For exporters"), which were the ONLY registration CTA a visitor met above the fold on a phone — nothing replaces that yet — and the always-open category rail (lg+). The old hero is intact at `/landing-2` and in git history. | Owner rebuilding it in stages | Partial |
...
in another milestone** (M3 search, M4 enquiry, store badges — the legal pages SHIPPED 2026-08-23) still must not be a
