...
The repo contains an **older copy** of the plan docs (`M2.md`, `m3.md`, `Models.md`, `Category.md`, `Search.md`, `m3-seo-rules.md`, plus charts). Those are stale. The current documents are:

- `MPX-Module2-Catalogue-Full.md`
- `MPX-Module3-COMPLETE-MEMO.md`
- `MPX-Module3-Discovery-Full.md`
- `MPX-Module3-SavedItem-Model.md`
- `MPX-Module3-Search.md`
- `MPX-COMPLETE-BRAIN.md`
- `MPX-SEO-Rules.md`

**Precedence, highest first:** (1) Part A of this document, (2) the current docs above, (3) the older repo copies — which you should treat as reference only, never as a source of truth.
...
- `SavedItem.orgId` (see A13 — saving is not buyer-only, so this one stays generic)
...
`Product`, `Category`, and `Organisation` each get a unique, indexed `slug`. Generation rules are in `MPX-SEO-Rules.md` §1 (lowercase, hyphens, strip specials, short unique suffix on clash, immutable once set).
...
## A13. Saving is open to any org

Any organisation can save products and suppliers — exporters buy too, and M3 already allows them to search.

`SavedItem` therefore uses `orgId`, not `buyerOrgId`. Everything else about the model is unchanged: polymorphic `targetType`/`targetId`, unique compound index on `(orgId, targetType, targetId)`, index on `orgId`, ownership-scoped reads and deletes.
...
- **SEO** — slug URLs, canonical tags, `noindex` on filtered and search URLs, dynamic sitemap of active entities only, JSON-LD with no private data. Full rules in `MPX-SEO-Rules.md`.

---

# Part C — Build order

## Prerequisite gap — Organisation / company profile
...
It is **not scoped and not decided**. Raise it before starting step 1; do not design or build it on your own initiative, and do not quietly proceed past it.
