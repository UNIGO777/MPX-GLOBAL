...
6.10 PUBLIC vs PRIVATE (WHITELIST projection; new fields default private): 
     - Seller public: name, **slug** (/supplier/:slug), logo, description, **`verified` boolean + verifiedAt** (the tick — never raw kycStatus/rejected), general location (city not street), **establishedYear**, active catalogue, member-since, product count. *(business type + categories = cancelled 2026-07-30.)* Private: KYC docs, **raw kycStatus**, direct contact (via M4 only), exact address, **website**, buyerSide/exporterSide, internal IDs, financial.
