...
| B7 | **App: show chat attachments** (D9 images + D10 documents) | `app/` chat thread screen | 🔴 Not built. The web shows both; the **app renders neither** — an app user sees only the line of text that came with the file, with no sign anything was attached. The API already returns `attachment.kind` / `url` / `name`. |
