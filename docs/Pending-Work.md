...
| B8 | **In-app notifications — roadmap Step 2 (quote Module 8)** | `Notification.js` skeleton · `my-plans/phase1-remaining/ROADMAP.md` | ⏸ **ON HOLD — owner, 2026-09-24 ("keep step 2 also in pending").** Not started. Covers: bell + notification centre (web + app), read/unread, which events feed it, admin per-type on/off, delivery tracking/retry, WhatsApp (quote: "partial"). Still red-alert guarded (D5) — raise it when the owner says go. B2 above is part of the same step. |
