...
| Claim | Reality |
|---|---|
| "The app is fully red (all 34 screens)" | `app/src/theme/colors.js:14` is `#2A4DE0` — royal blue. `#CE061A` appears in exactly two files repo-wide (`web/tailwind.config.js`, one design doc) and `git log -S` shows it never entered `app/`. The app was never turned red; there is no two-brand-colour problem, only one trial page. |
| "`OTP_DEV_FIXED_CODE` in production lets anyone into any account" | **The variable does not exist.** The real one is `OTP_DEV_PRINT`, which *echoes the code to the terminal* — not an auth bypass. Double-gated: `env.NODE_ENV !== 'development' \|\| !env.OTP_DEV_PRINT` (`otp.sender.js:79`), and the transform accepts only the literal string `'true'`. A log-leak to strip before launch, already on the close checklist. |
