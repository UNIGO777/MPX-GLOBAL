...
- **2026-09-25 — App: new brand artwork, a real upload key, and bank details on the phone.** Three
  ...
  - **Artwork** — every icon regenerated from `web/public/brand-logo.png`, so the app and the web now
    have ONE source for the mark. Icon (white plate: iOS forbids transparency), splash
    (transparent), Android adaptive foreground at **0.58 coverage** because the system crops the
    foreground to a circle and only the inner ~66% is guaranteed visible, a **monochrome** silhouette
    built from the alpha (themed icons are tinted by the system, so only shape survives), and the
    in-app `ColoredLogo.jpg` / `LogoWhite.png`. `adaptiveIcon.backgroundColor` was `#EAEEFF` — a pale
    BLUE left from the old brand — now white.
  - 🔴 **Changing `assets/` alone does NOTHING.** `android/app/src/main/res/mipmap-*` is generated by
    `expo prebuild`; those files were dated 22 August, so the first rebuild shipped the old icons and
    the build log looked perfectly healthy. Verified by unpacking the APK, which is how it was
    caught. `expo prebuild --platform android` then regenerated them.
  - **Upload keystore** (Play App Signing — owner's choice, so Google holds the app-signing key and
    can reset this one if it is lost). 4096-bit RSA at `app/credentials/`, passwords in a gitignored
    properties file, never in chat or git.
  - 🔴 **Signing lives in a CONFIG PLUGIN, not in `android/app/build.gradle`.** `app/android/` is
    gitignored — a hand edit there survives until the next prebuild and then vanishes, and nobody
    notices because a debug-signed release still builds and installs; it just cannot be uploaded to
    Play. The plugin re-applies on every prebuild, which this session then proved: prebuild wiped
    `android/` and the signing came back by itself.
  - ⚠️ **PKCS12 has no separate key password.** keytool silently uses the store password for the key,
    so the two different values written into the properties file produced
    *"Get Key failed: Given final block not properly padded"* — which reads like a corrupt keystore
    and is really a mismatch. Aligned, and noted in the file.
  - **App bank details** — `BankAccountsScreen` on the exporter's profile: list (masked), add, edit,
    remove. Same rules as the web: display-only (C1), the full number never comes back, and a
    quotation snapshots it at send. Exporter-only in the UI; `/me/bank-accounts` refuses any other
    role outright, so the hidden row is convenience, not the control.
...
- **2026-09-25 — More tests: web 79, app 16 (new); CI covers the app.**
  - **Web +25:**
    ...
    - chat attachment rules (docx/xlsx/pdf only, no .doc/.docm/.zip, 8 MB);
    - quotation amount-in-words (Indian and international), expiry and shipping strip;
    - buyer signup step 1 (local checks, trimmed identity, role buyer, duplicate → "Sign in instead").
  - The document-name list moved from the email service into `models/enums.js` (`KYC_DOC_LABELS`) so the parity test can import it.
  - **App:** `vitest@3` added (dev only); `app/node_modules` was installed on this machine with `npm ci`. Tests cover `config/env.js` (a release build refuses http, even localhost; a dev build allows only localhost/LAN; missing/relative/credentialed URLs refused), logger redaction, money, masking and validation. The CI workflow gains an `app` job.
- **2026-09-25 — Dependencies + CI (item 6) and more web tests (item 5).**
  - **firebase-admin 13 → 14.5.0.** v14 **dropped the namespaced API** (`admin.credential`, `admin.messaging`), so `push.client.js` now uses `firebase-admin/app` + `firebase-admin/messaging`, and the test mocks were updated to match. v14 needs **Node ≥22**, so `package.json` engines moved from `>=20` to `>=22` (Node 20 is end-of-life). **The VPS must run Node 22+.**
  - Audit now: backend 0 high / 2 moderate (uuid via gaxios 6 inside `@google-cloud/storage`; no upstream fix, and the vulnerable path isn't used); web 0 high / 2 moderate. `npm audit fix` crashes with an npm internal error, so updates are done package by package.
  - **CI:** `.github/workflows/ci.yml` (backend with Mongo 7 + Redis 7 services, lint, audit at high, tests; web lint, tests, build, audit). **Dependabot:** `.github/dependabot.yml` (backend + web weekly and grouped; app security-only; Actions monthly). Tracker **B9 → Done**.
  - **Gotcha found by a no-`.env` dry run:** 12 backend tests silently depended on the developer's `.env` (`CORS_ORIGINS` for the web-cookie tests, `EMAIL_LOGO_*` for the email-logo test). They're now pinned in `tests/setup.js`, and the suite passes with no `.env` (1394/1394).
  - Backend `testTimeout`/`hookTimeout` raised from 30 s to 60 s. An argon2-heavy auth test occasionally crossed 30 s under load (twice today, never alone). This was the likely cause of the earlier unexplained single failure.
  - **Web tests: +12 (now 54 in 12 files):** sign-in (portal sent, generic refusal, required fields); Support queue gating on `support:view_all`; supplier-request detail (picker only with `lead:assign`, "Take this request" self-take, no staff-list call); staff Products (`product:read` vs `product:takedown`; the D6 pending row with deletion paused; decline needs a reason and sends it).
- **2026-09-25 — B2 built: "More documents needed" email (email event 5, approved 2026-08-21).** `notifyDocumentsRequested` in `emailNotifications.service.js`, fired from `verification.service.js` `requestDocuments` next to the existing in-app notice. It goes to the company's account on the reviewed side. Subject: "More documents needed for your MPX Global verification". The body names each document (`DOC_LABELS`, which mirrors the web's `DOC_TYPE_LABELS`) and **includes the staff note** (owner's choice). No link, the employee is never named, the note is HTML-escaped, and a send failure is swallowed. Tests: +3 in `email-notifications.test.js`, plus a wiring assertion in `kyc-rounds-requests.test.js`. ⚠️ The staff note is free text: an email client may auto-link a URL typed into it, so staff shouldn't paste links there.
- **2026-09-25 — Credentials (owner):** the `.env` in git history is the developer's own DEV file, and production has a separate `.env` that was never committed. So no credential rotation is needed for launch. Recorded in tracker E6, `secrets-and-hygiene.md` and Pending-Work. The history purge is ready in the scratchpad clone for the owner to force-push. A secret scan is still due before handover.
- **2026-09-25 — Web tests added (owner: "do no. 5", chose "Logic + screens").** New dev-only packages, told first: `vitest@3` (the web is on Vite 5; the backend's vitest 4 needs a newer Vite), `jsdom`, `@testing-library/react`, `@testing-library/dom`, `@testing-library/user-event`. New files: `web/vitest.config.js` (kept apart from `vite.config.js` so the dev proxy never runs under test), `web/tests/setup.js`, and `npm test`. **42 tests in 8 files:**
  - logic: the permission catalogue and prerequisite map **checked against the server's own `config/permissions.js`** (so web and server can't drift), `withDependencies` chains and cascade, money minor units, `cp()` console paths, `roleHome`/`can`, `apiError`/`isErrorCode`/masking/format;
  - screens: route guards (restoring, signed out, must-change-password, wrong role), **Supplier requests gating** (no Unassigned card, no Assignee filter and no staff-list request without `lead:view_all`), the D6 `UnblockRequest` (10-char minimum, trimmed send, server refusal, pending/declined/7-day states), `NotificationItem`.
  - 🐛 **The tests found a real bug in today's code:** `UnblockRequest` put the whole `apiError()` object into state and React crashed rendering it on any server refusal. Fixed (`.message`).
  - ⚠️ Gotcha: a vitest 3 `vi.fn` that *rejects* surfaces as an unhandled rejection even when the code handles it (the spy's own result tracking). The unblock test uses a plain function mock for that reason.
  - Not covered yet: most pages (forms, chat, quotation, admin tables) and the app (no test runner in `app/`).
- **2026-09-25 — DECISION: no field-level encryption for the bank account number (tracker E1, owner: "not doing it").** A plan was offered: AES-256-GCM with a `FIELD_ENCRYPTION_KEY`, covering `ExporterBankAccount.accountNumber` and `Quotation.bankSnapshot.accountNumber`. The owner declined, so nothing was built. The number stays plain text in MongoDB behind `select:false`, last4-only responses/audit and log redaction. The tracker E1 note records the decision; the status stays *In progress*, because TLS and optional disk encryption on the VPS are still possible. Client sign-off recommended, as E1 is a quoted control.
- **2026-09-25 — Security tracker filled in (owner: "attend no. 6 and complete it").** `docs/security-tracker.xlsx`: all 58 controls now carry a status, evidence (files + named tests), a date and notes, each checked against the code. The result is **20 Done, 23 N/A, 7 In progress, 8 Not started**. N/A covers Phase 2 escrow/contracts/payouts, and A4 2FA was waived by the owner. What's left is mostly deploy/handover: B5 TLS, C10/E4 the DB-level insert-only audit grant, E5 staging, F1–F8. Three items are real gaps:
  - **E1:** the exporter bank account number (quotation module) is stored in plain text, not encrypted.
  - **E6:** a `.env` with test values is in git history (`a486611`). Rotate everything ever committed and run gitleaks/trufflehog before handover.
  - **G2:** app biometric unlock isn't built.

  Also: **B9 dependency audit.** The 3 backend HIGH findings were fixed by `npm update multer nodemailer ip-address` (multer 2.4.0, nodemailer 9.1.1, ip-address 10.7.2), all within their existing semver ranges. `npm audit fix` itself crashed with an npm internal error and changed nothing. That leaves 0 high/critical and 9 moderate, 8 of them needing a MAJOR firebase-admin upgrade (owner decision). The full backend suite passes, 1391/1391. **Corrected:** `secrets-and-hygiene.md` still said `.env` is tracked, but it has not been since `77f7208`.
- **2026-09-25 — D6 seller "request unblock" BUILT, web + app (owner: "build and test no.3 also build for app"; decisions: pause the purge, web notices both ways, 7-day re-ask).**
  - **Server:** `takedown.appeal` on Product; `POST /products/:id/unblock-request` (exporter, ownership-scoped, `unblockLimiter`); `POST /admin/products/:id/unblock-request/reject` (`product:takedown`, reason required); approve = existing restore (audits `unblockRequestApproved`, notifies the seller); `status=requests` list filter + `purgePaused`; `PURGE_NOT_PAUSED` shared by the purge job and the nearing-purge filter.
  - **Notices:** `unblockNotifications.service.js` sends the two new notice types; reviewer notices clear on decision.
  - **A9:** the seller never sees who decided.
  - **Web:**
    - staff Products page: filter "Unblock requested", chip, "Deletion paused" line, request block in the drawer, Decline modal, "Approve & restore";
    - seller: `UnblockRequest.jsx` inside `BlockedBanner` plus a list chip;
    - notice icon group `product`.
  - **App:** `UnblockRequestBox` on the edit screen plus a list chip. No app notices, because the app centre is still guarded.
  - **Checks:**
    - tests: `d6-unblock-request.test.js` (11), and the related moderation/purge/dashboard suites pass;
    - browser-tested end to end on 3100/5273 (take down → request → staff notice/filter → decline → seller notice + 7-day wait), and the test product was restored afterwards;
    - the app is parse + lint only, not run on a device.
  - Staff reports count `product.unblock_reject` under takedowns.
- **2026-09-25 — DECISION: Super Admin TOTP 2FA (D4) DROPPED (owner: "no need of 2fa").** The Super Admin keeps password + OTP, the same two factors as all staff. It's no longer a close-time item. Updated `auth-sessions.md` (A4), `remind.md`, `secrets-and-hygiene.md`, `Note.md` D4 + close checklist, and `Pending-Work.md`. ⚠️ A4 is one of the controls listed in the client's quotation security section, so the tracker row should read *waived by owner* rather than done.
- **2026-09-25 — DECISION: no month-1 / month-2 barrier; the whole project is in wrap-up (owner).** Bucket A is no longer deferred and no longer a red-alert trigger. **Quotation & negotiation is BUILT** (web + app, per the owner; confirmed in code: `quotation.routes.js`, `QuotationBuilder.jsx`, app `QuotationCard`, `quotations.test.js`). D4 TOTP moves from "month 2" to **due before handover**. Still guarded: Bucket B / Phase 2, D3/D2, and the owner's notification limits. Updated in the same pass: `scope-guard.md`, `remind.md`, `month1-not-doing.md` (banner), `Pending-Work.md` §6.
- **2026-09-25 — Correction: the APP organisation-claim screen IS built** (commit `a6b4116`, 2026-09-24: `ClaimOffer.jsx` + `SignupCompanyScreen.jsx`, all §A21 rules incl. rule-7 read-only profile/KYC). UiWebNotes row 57, `remind.md`, and Pending-Work still called it a stub, and I repeated that to the owner from the docs without checking the code. All three fixed. It's still not device-tested.
- **2026-09-25 — 9th email event: "ticket re-opened" → company (red alert raised, owner "yes").** `notifyTicketReopened` in `emailNotifications.service.js`, called from `setTicketStatus` when STAFF re-open a resolved ticket, alongside the existing in-app notice. It goes to the account that raised the ticket, exactly like events 7/8: fire-and-forget, no link, no message text, never names the employee. A company re-opening its own ticket sends nothing. Tests: +2 in `email-notifications.test.js` (content/no-link, and a swallowed send failure) + the missing-raiser skip. D5 guard moved: a **10th** email event needs a fresh alert (`remind.md`, `scope-guard.md`, `Note.md` updated). Also refreshed `docs/Pending-Work.md` (consent decided, month-2 list trimmed of built items, B12/B13 added).
- **2026-09-25 — One-time permission backfill (owner: "you can do it for this time"; a ONE-OFF exception to CLAUDE.md's no-migration rule, not a precedent).** A throwaway script (`scripts/backfill-permission-prerequisites.mjs`, since deleted) did, for every employee at once, what saving them on the Staff page would do: it adds the prerequisites of grants they already hold (`withPrerequisites`). It never removes a grant, and never adds anything not implied by an existing one. It's a dry run unless `--apply`, idempotent, and prints ids only. Each change writes an append-only AuditLog `employee.permissions.backfill` with a system actor. Run on both dev DBs (shared test cluster: 1 of 9 employees; local test DB: 1 of 7), and both got + `organisation:read`. The re-run found nothing. **Production does NOT need it (owner, 2026-09-25):** prod starts on a fresh database, where every employee is created through `createEmployee`/`setEmployeePermissions`, and those already apply the prerequisites. So the script was deleted after this run; there's nowhere left to run it. It deliberately does NOT give "See all tickets" or "See all supplier requests" to people who only had view or route. That would undo the owner's own-queue-only decision, so those stay a per-person choice on the Staff page.
- **2026-09-25 — Staff permission fixes, round 2 (owner: "for 5 option b and fix all others also").** (1) **Prerequisites are now automatic.** "Verify exporters", "Approve buyers" and "View KYC documents" each bring "View organisations" (their screens load the company record). "See all tickets" → "View tickets", "Assign tickets" → "See all tickets", and the reply/status grants → view. The server adds them transitively on create and on save (`PERMISSION_REQUIRES` + `withPrerequisites` in `config/permissions.js`); the Staff page ticks them the same way. The Verification menu and page are now gated on `organisation:read` alone. (2) **Supplier requests are split like tickets.** `lead:manage` = only requests assigned to you, on every path (list, detail, timeline, status, route, notes, overview; others are 404). New `lead:view_all` "See all supplier requests" = everything plus who holds it, and can TAKE an unassigned one. New `lead:assign` "Assign supplier requests" = give to anyone (it requires view_all). The web hides the Unassigned card and the Assignee filter without view_all, and LeadDetail shows the owner read-only plus a "Take this request" button without assign. (3) **Staff-name lists are locked.** `/admin/support/assignees` needs `support:view_all` and `/admin/leads/assignees` needs `lead:view_all`, and the web only asks when allowed. (4) **Reassigning clears the previous holder's "assigned to you" notice** (tickets and requests). ⚠️ **No data migration (CLAUDE.md forbids one):** staff saved BEFORE today keep their old grants. Anyone who should still see the whole ticket or request queue needs "See all tickets" / "See all supplier requests" ticked on the Staff page, and old reviewers get "View organisations" the next time they are saved. Catalogue is now 24 permissions. Tests: support-tickets, lead-routing, userManagement, m5-foundation updated; full suite 1378/1378.
...
- **2026-09-25 — Ticket visibility: new permission "See all tickets" (`support:view_all`) + dashboard fixes (owner).** **Decision (owner):** staff WITHOUT `support:view_all` see ONLY tickets assigned to them; with it, every ticket, assigned or not, and who has each. Assigning to others stays `support:assign`, which now REQUIRES `support:view_all` on the Staff page (you can't hand out what you can't see; `withDependencies` now follows chains both ways). Supplier requests are unchanged for now (owner). Server: `canSeeAllTickets` / `ticketScope` in config/permissions.js, ONE rule on every staff ticket path — list (the assignee filter can only narrow), open, timeline, reply, status, assign, internal notes on a ticket, and the overview counts (`scope: 'mine'|'all'`, no unassigned count for own-only). Someone else's ticket is a 404. Catalogue: 22 permissions (m5-foundation test updated). Tests: 2 new visibility tests (every path 404s for others' tickets; own works; view_all sees all); existing fixtures that act on arbitrary tickets now also hold view_all. ⚠️ **Existing employees who held "View support tickets" now see only their own tickets until granted "See all tickets".** Web: Support page drops the Unassigned card + Assignee filter and says "The tickets assigned to you" for own-only staff; the dashboard Support panel drops Unassigned. **Dashboard (owner):** "You don't have any access yet" was computed from the PLATFORM tiles only, so a support-only employee (Asha) was told she had no access and her own panels were hidden. Now it means zero permissions, and it is a notice ABOVE "My work", which every employee always sees. **Grey holes (owner screenshot):** `CountStrip` was a fixed 4 columns, so a count removed by a permission left a grey patch. It now sizes to its counts (odd last cell spans on phones); the Support page cards do the same. Also: opening a ticket clears the staff member's "assigned to you" notice too (own refKeys `staff-ticket-assigned:` / `lead-assigned:`); notification reads/clears refuse a missing userId (an undefined key would have matched everyone's rows).
- **2026-09-25 — WEB notification centre built (B8; owner override of the 2026-09-24 hold after a red alert; "no app touch").** Server: the `Notification` skeleton filled in (USER-scoped; type/title/body/link/refKey/count/at/readAt; 90-day TTL). `notification.service.js`: `notify` never throws and never fails the action it follows; it coalesces on `refKey` (one unread row per person per thing); recipients via `companyUserIds` / `staffUserIds`. Routes `GET /notifications`, `GET /notifications/unread-count`, `POST /notifications/:id/read`, `POST /notifications/read-all`, all four roles, always the caller's own (a foreign id is 404). A per-user socket room pushes a content-free `notifications:changed`. **Events:** verification (approved/rejected, documents requested, revoked, profile change approved/rejected); tickets (staff reply, resolved, re-opened, auto-closed to the company; assigned-to-you and company-replied to staff); supplier requests (in progress, connected with the chat link, closed; assigned-to-you); chat (new enquiry to the seller; new messages grouped per thread, skipping anyone viewing it, never the text; platform warning to both sides); new KYC documents to reviewers of that side (grouped per company). Opening the ticket / thread / KYC page clears its notice. Web: `BellIcon`; `NotificationBell` in the ConsoleShell top bar (every role and width; panel shows the latest 10, mark all read, see all); `/buyer|exporter/notifications` and console `/notifications` page (All/Unread in the URL, show older). Companies update live over the chat socket; staff poll every 60 s and on focus. Tests: `b8-notifications.test.js` 9/9 (ownership 404, coalescing, ticket reply + clear-on-open, assignment incl. self, company reply to assignee, verification link, chat grouping without text). Browser-verified: live bell count, click-through clears, phone panel, staff/admin pages. Docs updated: scope-guard, remind, Note D5, Pending-Work B8.
- **2026-09-25 — Chat count missing on phones and tablets (owner report: "count on chat tab not showing").** Root cause: only the desktop sidebar (lg+) rendered the nav badges; the tab row used below lg rendered bare labels, so Chat (unread threads), Saved and Help & support (new replies) had no count there. The server count and the floating chat button were always right; investigated and confirmed. Fix in `ConsoleShell.jsx`: a shared `CountPill` and a `NavBadges(item, variant)` used by BOTH the sidebar and the strip, so they cannot drift again. The strip variant is a solid white pill on the red bar. Verified at 390, 900 and 1440: "Chat ①" in the visible menu; it clears after the thread is read and returns live (socket, no reload) when the other side writes.
- **2026-09-25 — Phone pass over every drawer and dialog (owner: "in all other such windows search if any ui defect fix it").** Fixed at the shared primitives, not per page. (1) **`Modal` panel is now height-capped and scrolls** (`max-h-[calc(100dvh-2rem)] overflow-y-auto`). With the page behind locked, a tall dialog on a phone used to run off the bottom with its buttons unreachable. Phone padding is tighter too (p-5 / p-6). (2) **`Modal` and `Drawer` footers**: button and link labels never wrap (`whitespace-nowrap`); footers `flex-wrap`; below sm, buttons `grow` to fill their row (wrapping to a new row when two do not fit). The drawer footer also has phone padding. (3) KYC viewer (4 dialogs) and verification-queue reason counters no longer break "0 / 500" over two lines. (4) Mobile numbers are shown grouped via a shared `formatMobile` (lib/format.js) in the Users detail drawer, Users and Staff rows, and My account (Account.jsx now imports it instead of a local copy). Checked at 390×740: Featured add/edit, Staff add + access, Users detail, Products detail + take-down, Category new/sub, Audit + Error entries, Support ticket details, Org block, KYC request, Mark as solved, product delete. Desktop unchanged. Not opened in this pass: LeadDetail drawer, Company-profile pending-change dialogs, Attribute manager drawer. Quotation windows were left alone (Bucket A1, teammate's code).
- **2026-09-25 — Drawer footers on phones (owner screenshot).** The New ticket and Find-a-supplier drawers squeezed the status hint, Cancel and the primary button into one row at 390 px, so "Send ticket" wrapped onto two lines. New shared `DrawerActions` (components/ui/Drawer.jsx): on phones the hint gets its own line, then two equal-width buttons that never wrap; from sm, one row (hint left, buttons right). Both drawers use it.
...
- **2026-09-25 — My account (admin + staff) redesigned (owner).** One profile card: avatar, name, role, and facts for email, mobile (Indian numbers grouped for display), member since and last sign-in. `GET /auth/me` now also returns the caller's OWN `memberSince` (createdAt) and `lastLoginAt` — own record only, never on login/refresh payloads; test added in m1-auth-session, 20/20. The name is no longer shown twice. Password is folded to one row with a "Change password" button (the form opens with focus on Current password; Cancel clears it; success collapses it with a flash) plus a "forgot your current password?" hint. Your access: an "N of 21 permissions" count for employees plus the grouped list and an "ask a super admin" footer; for the super admin, "Everything" plus direct links to the two things only they can do (Staff & permissions, Platform settings). Removed an unreachable temporary-password branch: a `mustChangePassword` account is routed to the dedicated "Set a new password" screen before any console page. Full backend suite: 1312/1312.
- **2026-09-25 — D8 extended: three new platform settings (red alert raised; owner confirmed "Support hours, Ticket auto-close days, Company footer details"; declined the company AI allowance).** `Settings` gains `supportHours` (≤80), `ticketAutoCloseDays` (3–90, null = default 14), `companyLegalName` / `companyAddress` / `companyLinkedinUrl` (https LinkedIn only — it becomes an href on every public page). All are in the `.strict()` validator and audited like the rest. **Auto-close:** `effectiveTicketAutoCloseDays()` (never throws, falls back to 14) drives the nightly job; each auto-closed ticket stamps `autoClosedAfterDays`, so its "no reply for N days" line stays true later; staff ticket views carry a server-computed `autoCloseAt`, so the web no longer keeps its own 14 (`autoCloseDate` removed); the overview returns `autoCloseDays` for the "Closes itself after N days" hint; log rows carry `afterDays`; the resolved email uses the real count. **Public:** `GET /public/support-contact` is now `{ support: {email, phone, hours}, company: {name, address, linkedinUrl} }`, widened consciously and pinned in support-contact.test.js. Help page (web + app) shows the hours; the site footer shows name, address, LinkedIn (new tab) and "© <year> <name>"; every email foot adds "(hours)" and a plain-text "name, address" line, never the LinkedIn link (emails stay link-free; tested). Settings screen: a hours field in Support contact, a Support tickets row (In force N days, a warning when lowering it — "closes at the next 03:30 run", back-to-default), and a Company details row with a live footer preview; all validated in the form. Tests: d8 +3, support-contact rewritten, support-tickets +1, email +1. Dev DB: days reset to the default; dummy hours/company left in place. Note.md D8 + remind.md updated.
- **2026-09-25 — Platform settings screen redesigned (owner: "fix setting screen"). Same two settings; no field added (D8 exclusions untouched).** Each setting has an explanation column plus a control card. AI guest limit: a plain "In force: N a day — your setting / the server default" status, a WARNING when nothing is in force ("AI costs have no daily cap"), a "searches / day" suffix, and "Go back to the server default". Support contact: a live "What people will see" preview, and the stale always-on "nothing is published" sentence now shows only when nothing is published. Checks before saving that mirror the server (whole number 1 to 1,000,000; email shape; phone up to 40 chars). The save bar says "You have unsaved changes" or "Everything is saved" and has Discard. The header link "Last changed <date, time>" opens `/admin/audit?action=settings.update` (it used to say only "Thu"). Verified in the browser: validation, discard, save 500, revert to default, history link, phone width. The dev DB limit is back to the default.
- **2026-09-25 — Featured page issue pass (owner: "test for other issues and fix") + console-wide per-page access screen.** Tested end to end: add with a schedule, Starts/Ended statuses, inverted dates, bad link, clear link, reorder, replace image, remove, phone width, keyboard. Fixed: (1) the inverted-date and bad-link rejections said only "Invalid request." Both are now checked in the dialog in plain words under the field, the same rules as the server (which stays the authority), with Save disabled; a server rejection that names `linkUrl`/`startsAt` is reworded too. The collapsed Position & schedule section opens itself on a date error. (2) The section tabs now support arrow keys, Home and End (WAI-ARIA tabs, roving tabindex). (3) **Console-wide:** a staffer typing or bookmarking a console page without its permission got a half-drawn page (tabs, an Add button) over a "couldn't load" 403. `CONSOLE_ROUTES` entries now carry `perms` — the same any-of lists as the sidebar — and a `PermGate` shows `NoAccess page` ("This page isn't part of your access", with links to the dashboard and to My account). Presentation only; the server 403 is still the lock. Verified as Staff Tester across all 14 console pages: each either loads with zero 403s or shows the no-access screen.
- **2026-09-25 — Featured: type-to-search picker + product rows with context (owner: "search is not type to search").** New staff route `GET /admin/featured/candidates?kind=&q=` (featured:manage, zod-validated, declared before `/:id`). It does a WORD-PREFIX match on the name (regex-escaped), so "twi" finds "Twill". **Gotcha:** the public search is `$text` (whole words, stemmed; A26), so it found nothing mid-word. The route offers only what the landing could show and returns id/name/image/meta/verified only (derived tick, never kycStatus). Empty q gives the newest; max 8; maxTimeMS 2 s. The Add dialog searches 300 ms after typing (no Enter), shows the newest first, marks "Already featured", and folds Position & schedule away as optional. Admin rows now carry `target.context` (product: seller + category; category: parent; supplier: country), names only. Tests f5b 44/44 (+4).
- **2026-09-25 — Featured: banner preview now matches the landing (owner: "the preview is wrong").** It showed the whole image uncropped, with its own text styling, in a 448 px window. New shared `BannerSlide` (FeaturedStrips.jsx) draws one banner; the landing strip and the admin preview both use it, so they cannot drift. The preview has a Desktop / Phone switch. Each draws the banner at its real landing size (desktop 1312×224 = a 1440 screen; phone 358 wide at 16:7) and scales the whole frame down to fit, so crop and text size are exact ("shown at 65%"). `BannerSlide` takes an explicit `size` because Tailwind breakpoints follow the viewport, not the frame. `Modal` gained an opt-in `wide` (max-w-4xl); every existing dialog is unchanged.
- **2026-09-25 — Featured: the On/Off switch never worked (owner report).** **Gotcha:** the shared `Switch` calls `onChange` with the CLICK EVENT, not a boolean. Featured passed it straight on as `{ active: <event> }`, which axios cannot serialise, so the request failed in the browser ("Could not update this slot.") and never reached the server. This predates the 09-24 redesign. Fixed at the call site by flipping from the row (`active: !row.active`), which is the contract the Category and Attribute managers already follow. Verified: switching off drops the banner from the landing (3 → 2), and switching on brings it back.
- **2026-09-25 — No more guest-UI flash on reload for signed-in visitors (owner) + banner height.** The public pages drew the GUEST version while `AuthContext` was still restoring the session, then swapped: "Sign In / Get Started", the landing's "Start sourcing / Sell to global buyers" cards, ProductCard's "Send enquiry" for an exporter, and Help's "Sign in to raise a ticket". All of them now wait for the existing `restoring` flag. The header holds an invisible pill; the landing panel shows a neutral placeholder; ProductCard holds the enquiry strip's space invisibly (no pop-in for guests, no flash for exporters); ProductListCard shows its same-sized fallback link; Help reserves the row. Verified with a MutationObserver from the first paint: a signed-in exporter never sees any guest text; a guest still gets all of it. Banner strip: fixed heights from sm up (208 / 224 / 256 px) instead of an aspect ratio that grew to ~340 px on a wide monitor; phones keep 16:7. The admin Add-banner window now recommends a 1800×400 image and warns that page screenshots crop badly. ⚠️ `LandingBlue.jsx` not updated.
- **2026-09-25 — Featured content back on the landing page (owner: option 1 + "if no featured is there then default"; banner placement: strip under the hero).** It had been OFF the landing since 2026-08-23, so `/admin/featured` did nothing visible (found while verifying). `FeaturedStrips.jsx` is now pieces (`useLandingFeatured`, `BannerStrip`, `SupplierCard`) that `Landing.jsx` places into its own sections, each falling back to the default: **banners** = a rotating strip UNDER the hero (the hero is unchanged; arrows + dots; 6 s auto-advance that pauses on hover/focus and is off under reduced motion; none → no strip); **products** = a "Featured products" row above "Recently listed" (none → absent); **categories** = featured first in "Browse by category", topped up with the usual categories to 12 (none → exactly the old grid); **suppliers** = a "Highlighted suppliers" row (none → absent; deliberately no default, because the owner removed the default suppliers section on 09-23). Loading/error = defaults. New `lib/featuredLimits.js` (banner 24 · product 10 · category 12 · supplier 8) is read by BOTH the landing and the admin page. Admin shows a live slot past the limit as "Not shown" (it said "up to 24" while the page showed 8), and the tab counts and section descriptions now say where each kind appears. Also fixed: nested `<li>` on the landing (ProductCard renders its own `<li>`; "Recently listed" wrapped it in another). ⚠️ `LandingBlue.jsx` (`/landing-blue` comparison copy) NOT updated. Verified in the browser at 1440/490: all four sections, 3-banner rotation in position order, no overflow. Dev DB test rows: 3 banners, 2 products, 1 category, 1 supplier.
- **2026-09-24 — Featured content page redesigned (owner) + two server fixes.** Page: tabs per section (`?kind=`) with live/total counts, a "View landing page" link, and one sentence per tab saying where the section appears. Each slot shows its position and ONE status (Live · Starts <date> · Ended · Switched off · Hidden · Removed), decided in the same order the public read filters. The row keeps just an On/Off switch plus a ⋮ menu (Edit / Position & schedule · Preview · Replace image, with a spinner on the thumbnail · Open public page · Remove). The Add dialog marks items that are "Already featured". There is now ONE edit dialog for every kind: position + "Show from/until" with Clear, plus the text and link for banners. Server: (1) `PATCH /admin/featured/:id` now accepts `startsAt/endsAt: null` to REMOVE a date. **Gotcha:** `z.coerce.date()` turns a bare null into 1970-01-01, so a null `endsAt` used to mean "already ended", silently pulling the slot off the landing page. (2) The service rejects a window inverted against the STORED other date (the validator only sees the dates sent). (3) The `featured.update` audit `after` now records every field it set, not just order/active (new rows only; nothing existing touched). Tests f5b 40/40 (+3). Dev DB now holds test slots: one banner and two products.
- **2026-09-24 — Error log screen + error window redesigned (owner).** New read-only `GET /admin/errors/summary` (`errorlog:read`, declared before `/:id`): counts for the last 24 h / 7 d / 90 d, plus the top 5 failing route PATTERNS of the week (ids collapsed to `/:id` by `routePattern`, capped at 2,000 rows); test added (f5 38/38). Page: a read-only badge ("kept 90 days"), then one SLIM overview strip (owner: the first version's big cards were too big). The counts act as period shortcuts, and the route chips filter by their fixed prefix. Also one support-reference search, Method / Status / Period chips (custom dates inline), a removable route-prefix chip, and rows grouped by day. The window has a status + route card with the message, a copyable support reference, "Signed in as", a company link (gated by `organisation:read`, via `cp()`), "Show similar" (route / status), and the stack with a copy button. The expired (404) state is kept. Still NO delete / clear / export. Gotcha seen in dev data: one 500 on `POST /auth/signup/organisation` is a Mongoose VersionError on `claimCandidates`, still to investigate.
- **2026-09-24 — Audit log screen + entry window redesigned (owner).** New read-only `GET /admin/audit/facets` (`audit:read`, before `/:id`) returns the actions and target types that ACTUALLY occur in the log (`distinct`), so the pickers can never fall behind the code (+1 test). Screen: a "Read-only · entries can never be changed or deleted" badge; searchable **Action** picker (was a typed exact name) and **Actor** picker; **Target** + **Period** chips (Any time / Today / 7 / 30 days / Custom → inline dates); organisation / single-target scopes shown as removable chips; entries grouped by day, each with a tone dot, action, target-type tag, "actor · role · on target" and a quoted reason. Entry window: action + raw code, reason, "Done by" / "On" cards (target links to its page when it still exists and the viewer holds the grant), a before/after table (permission changes shown as named + added / − removed), read-only **Show similar** filters (same action / same person / this target's history), technical trail (occurred, recorded, request reference, target id, IP, device), "this entry is permanent". `auditFormat.js` gained `entityTypeLabel()` and `roleLabel()`. Still no edit, delete or export anywhere. Browser-checked at 1440 / 490.
- **2026-09-24 — Cloud review findings addressed.** (a) "utils/messageText.js missing" — the file exists locally but is NEW and UNTRACKED; the review only sees the repo. It MUST be committed with the rest (see the untracked-file list given to the owner) or the backend will not start. (b) Organisation page: the UNBLOCK cascade result was lost when block status moved to the header — now a green "Unblocked · restore completed · N restored · N reopened" shows for 7 days after an unblock. (c) Team-scope rule deduplicated into `hasTeamScope(actor)` in `config/permissions.js`, used by the staff report and the ticket log so they cannot drift. (d) `messageView` derives the system kind once instead of twice. Affected suites 76/76.
- **2026-09-24 — Dashboard: Recent activity no longer sits a third wide next to empty space (owner).** The verification-queue panel hides when the queue is empty; the row now knows that (`showQueue`) and Recent activity takes the full width, laid out in two newspaper columns (newest down the first, then the second) with the single-column connecting line hidden.
...
- **2026-09-24 — Four owner decisions built.** (1) **"Connected by MPX Global" notice is one neutral line for both parties** ("MPX Global connected this buyer and supplier. Continue the conversation here as usual."). (2) **Country NAMES in enquiry messages** ("Delivery to: Australia"): new enquiries write the name (`composeEnquiryMessage`, via `Intl.DisplayNames` — no dependency); old stored messages are PRESENTED with the name. Both rules live in `utils/messageText.js` and are applied in the server's `messageView` + inbox previews — messages are append-only, so stored text is never edited, and web + app are fixed without client changes. (3) **A staff reply on a CLOSED ticket needs `support:status`** (it would re-open it) — 403 before any upload; the ticket page shows an explanation instead of the reply box. (4) **`reports:team` also widens the Support ticket log** to the whole team (the "Who" filter shows for them). Tests: new `message-text.test.js` (4), support-tickets +2; related suites 289/289. Browser-checked as the seller: neutral notice + "Australia".
- **2026-09-24 — Organisation page: "What a block does" rail card removed (owner: right side too long).** Its explanation already lives in the Block / Unblock dialog (where the decision is made; the card also showed to employees who cannot block). The only unique part — cascade progress / last result — moved into the block status notice under the header ("Blocked <date>: reason" + "Cascade running…" or "Cascade completed · N products taken down · N conversations frozen"). The rail is now Internal notes, Sides, Audit trail.
- **2026-09-24 — Employee onboarding + password change verified end to end in the browser (dev).** Super admin created test employee "Asha Menon" (support read/reply/status) → staff sign-in with the temporary password + OTP → forced "Set a new password" (nothing else reachable) → landed on `/staff/dashboard` with only her menu items → changed the password again on **My account** ("every other session signed out") → signed out → the previous password is refused ("Invalid credentials.") → the new one + OTP signs in to `/staff/dashboard`. Note: the dev test server runs `OTP_DEV_FIXED_CODE=false`, so codes come from its dev print; a throwaway local helper handed them to the browser without echoing them. The test account and its passwords are dev-database only.
